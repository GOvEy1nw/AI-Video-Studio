...
export function TimelineTrackHeaders({
  ...
  return (
    <div className="w-32 shrink-0 border-r border-zinc-800 bg-zinc-900 flex flex-col overflow-hidden">
      <div className="shrink-0 h-7 flex items-center px-2 gap-1.5 border-b border-zinc-700/50">
        <button onClick={onAddVideoTrack} className="text-2xs text-zinc-500 hover:text-zinc-300 flex items-center gap-0.5" title="Add video track"><Plus className="h-3 w-3" />V</button>
          ...
          return (
            <React.Fragment key={track.id}>
              {displayRow === audioDividerDisplayRow && (
                <div className="shrink-0 bg-zinc-700/60 relative cursor-row-resize hover:bg-blue-500/30 transition-colors group/divider" style={{ height: dividerHeight }} onMouseDown={resizeDivider}>
                  <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex items-center justify-center"><div className="flex flex-col items-center gap-px"><div className="w-8 h-px bg-zinc-500 group-hover/divider:bg-blue-400 transition-colors rounded-full" /><div className="w-8 h-px bg-zinc-500 group-hover/divider:bg-blue-400 transition-colors rounded-full" /></div></div>
              ...
              <div className={`group shrink-0 border-b border-zinc-800 text-xs relative ${track.type === "subtitle" ? "bg-amber-950/20 px-1.5 flex flex-col justify-center gap-0" : track.kind === "audio" ? "bg-emerald-950/10 px-2 flex items-center justify-between" : "px-2 flex items-center justify-between"}`} style={{ height: trackHeight }}>
                {track.type === "subtitle" ? (
                  <>
                    <div className="flex items-center gap-1"><MessageSquare className="h-3 w-3 text-amber-500/60 shrink-0" /><span className={`text-[10px] font-semibold truncate ${track.muted ? "text-zinc-600" : "text-amber-400/80"}`}>{track.name}</span></div>
                    <div className="flex items-center gap-0">
                      ...
                      <Tooltip content={track.locked ? "Unlock" : "Lock"} side="right"><button onClick={() => updateTrack(realIndex, (current) => ({ ...current, locked: !current.locked }))} className={`p-0.5 rounded-sm ${track.locked ? "text-yellow-400" : "text-zinc-500 hover:text-zinc-300"}`}>{track.locked ? <Lock className="h-2.5 w-2.5" /> : <Unlock className="h-2.5 w-2.5" />}</button></Tooltip>
                      <Tooltip content={track.muted ? "Show subtitles" : "Hide subtitles"} side="right"><button onClick={() => updateTrack(realIndex, (current) => ({ ...current, muted: !current.muted }))} className={`p-0.5 rounded-sm ${track.muted ? "text-red-400" : "text-zinc-500 hover:text-zinc-300"}`}>{track.muted ? <EyeOff className="h-2.5 w-2.5" /> : <Eye className="h-2.5 w-2.5" />}</button></Tooltip>
                      <Tooltip content="Delete track" side="right"><button onClick={() => onDeleteSubtitleTrack(realIndex, track.name)} className="p-0.5 rounded-sm text-zinc-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="h-2.5 w-2.5" /></button></Tooltip>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="flex items-center gap-1 min-w-0 overflow-hidden">
                      <Tooltip content={track.sourcePatched !== false ? "Source patched (click to unpatch)" : "Source unpatched (click to patch)"} side="right"><button onClick={() => updateTrack(realIndex, (current) => ({ ...current, sourcePatched: !(current.sourcePatched !== false) }))} className={`p-0.5 rounded shrink-0 transition-colors ${track.sourcePatched !== false ? track.kind === "audio" ? "text-emerald-400 hover:text-emerald-300" : "text-blue-400 hover:text-blue-300" : "text-zinc-600 hover:text-zinc-400"}`}>{track.sourcePatched !== false ? <CircleDot className="h-2.5 w-2.5" /> : <Circle className="h-2.5 w-2.5" />}</button></Tooltip>
                      <span className={`text-[10px] font-semibold truncate ${track.muted ? "text-zinc-600" : track.kind === "audio" ? "text-emerald-400/80" : "text-zinc-300"}`}>{track.name}</span>
                    </div>
                    <div className="flex items-center gap-0 shrink-0">
                      <Tooltip content={track.locked ? "Unlock" : "Lock"} side="right"><button onClick={() => updateTrack(realIndex, (current) => ({ ...current, locked: !current.locked }))} className={`p-0.5 rounded-sm ${track.locked ? "text-yellow-400" : "text-zinc-500 hover:text-zinc-300"}`}>{track.locked ? <Lock className="h-2.5 w-2.5" /> : <Unlock className="h-2.5 w-2.5" />}</button></Tooltip>
                      {track.kind !== "audio" && <Tooltip content={track.enabled === false ? "Enable track output" : "Disable track output"} side="right"><button onClick={() => updateTrack(realIndex, (current) => ({ ...current, enabled: !(current.enabled !== false) }))} className={`p-0.5 rounded-sm ${track.enabled === false ? "text-zinc-600" : "text-zinc-500 hover:text-zinc-300"}`}>{track.enabled === false ? <EyeOff className="h-2.5 w-2.5" /> : <Eye className="h-2.5 w-2.5" />}</button></Tooltip>}
                      {track.kind !== "audio" && <Tooltip content={track.muted ? "Unmute" : "Mute"} side="right"><button onClick={() => updateTrack(realIndex, (current) => ({ ...current, muted: !current.muted }))} className={`p-0.5 rounded-sm ${track.muted ? "text-red-400" : "text-zinc-500 hover:text-zinc-300"}`}>{track.muted ? <VolumeX className="h-2.5 w-2.5" /> : <Volume2 className="h-2.5 w-2.5" />}</button></Tooltip>}
                      {track.kind === "audio" && <button onClick={() => updateTrack(realIndex, (current) => ({ ...current, muted: !current.muted }))} className={`px-1 py-0.5 rounded text-[10px] font-bold leading-none ${track.muted ? "bg-red-500/80 text-white" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700"}`} title={track.muted ? "Unmute track" : "Mute track"}>M</button>}
                      {track.kind === "audio" && <button onClick={() => updateTrack(realIndex, (current) => ({ ...current, solo: !current.solo }))} className={`px-1 py-0.5 rounded text-[10px] font-bold leading-none ${track.solo ? "bg-yellow-500/80 text-black" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700"}`} title={track.solo ? "Unsolo track" : "Solo track"}>S</button>}
                      {tracks.length > 1 && <Tooltip content="Delete track" side="right"><button onClick={() => onDeleteTrack(realIndex)} className="p-0.5 rounded-sm text-zinc-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="h-2.5 w-2.5" /></button></Tooltip>}
                    </div>
                  </>
                )}
                ...
                }}><div className="mx-auto w-6 h-0.5 bg-zinc-600 rounded-full mt-0.5 group-hover/resize:bg-blue-400 transition-colors" /></div>
              </div>
            </React.Fragment>
          );
        })}
