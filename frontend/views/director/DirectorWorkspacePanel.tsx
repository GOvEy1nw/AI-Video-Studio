...
import { snapLtxFramesUp } from "@/lib/director-timeline";
import {
  formatDirectorError,
...
export function DirectorWorkspacePanel(props: Props) {
  ...
  const applyPromptAsset = (
    ...
    if (asset.type === "image") {
      ...
      const durationFrames = snapLtxFramesUp(
        sequence.output.durationFrames - previousPrefix + prefix,
      );
      const shift = prefix - previousPrefix;
  ...
  const revertContinueVideo = () => {
    ...
    const prefix = sequence.continueVideo.timelineDurationFrames;
  ...
  return (
    <section
      className="flex h-full min-w-0 flex-1 flex-col overflow-hidden border-r border-zinc-800 bg-background"
      tabIndex={0}
      onKeyDown={(event) => {
        if (
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-0 p-0">
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="flex h-full items-stretch">
            ...
            >
              ...
              <div className="p-2">
                <div className="mb-2 text-[14px] font-bold uppercase tracking-wide text-zinc-500">
                  Global Settings
                </div>
                <span className="flex gap-2 mb-2">
                  ...
                  {sharedKeyframe ? (
                    <label className="flex min-w-0 items-center gap-2 text-zinc-500">
                      <span className="flex-shrink-0 text-[11px] font-medium tracking-wide text-zinc-500">
                        Key Frame Strength
                      </span>
                      <span className="w-7 flex-shrink-0 text-[11px] text-right font-mono tabular-nums text-zinc-400">
                      ...
                      <input
                        ...
                        className="min-w-0 flex-1 accent-zinc-400"
                      />
                    </label>
                  ) : (
                    <div
                    ...
                    >
                      <span className="flex-shrink-0 text-[11px] font-medium tracking-wide">
                        Key Frame Strength
                      </span>
                ...
                <textarea
                  ...
                  className={`${inputClass} min-h-24 resize-none normal-case`}
                  placeholder="Add your global text prompt here…"
                />
              </div>
              <div className="h-0.5 bg-zinc-800 mx-auto w-1/2 my-4"></div>
              <div className="flex min-h-0 flex-col flex-1 p-2">
                <div className="mb-2 text-[14px] font-bold uppercase tracking-wide text-zinc-500">
                  Segment Settings
                </div>
                <div className="mb-2 text-[11px] font-medium uppercase tracking-wide text-zinc-500">
                ...
                <div className="grid min-h-0 flex-2 gap-2 mb-2">
                  <div className="flex min-h-24 min-h-0 min-w-0 flex-col">
                    {continueSelected ? (
                      <textarea
                        disabled
                        value=""
                        className="min-h-0 w-full flex-1 resize-none rounded border border-zinc-700 bg-zinc-800 p-2 text-xs text-zinc-100 outline-none disabled:cursor-not-allowed"
                ...
                <div className="grid min-h-0 flex-1 gap-2 mt-2">
                  <div
                    className={`relative flex h-full min-w-0 flex-col overflow-hidden transition-colors ${mediaDragOver ? "border-blue-500 bg-blue-950/30" : "border-zinc-700"}`}
                    onDragEnter={(event) => {
                      event.preventDefault();
        ...
        <div className="flex h-10 flex-shrink-0 items-center bg-zinc-950 px-3">
          <span className="w-44 font-mono text-[12px] tabular-nums text-amber-400">
            {timecode(playheadFrame, sequence.output.fps)} (Frame {playheadFrame})
          </span>
