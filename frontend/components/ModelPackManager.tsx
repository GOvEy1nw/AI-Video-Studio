...
export function ModelPackManager({ firstRun = false, onContinue }: ModelPackManagerProps) {
  ...
  useEffect(() => {
    void refresh();
    window.electronAPI.onModelPackProgress((data: unknown) => setProgress(data as PackProgress));
  ...
  const currentFile = progress?.file
    ...
    : progress?.packName ? `Downloading ${progress.packName}` : "Preparing download";
    ...
    <div className={firstRun ? "w-full max-w-3xl" : "space-y-4"}>
      <div className="space-y-1">
        ...
        <p className="text-xs leading-relaxed text-zinc-400">
          Packs include each model’s WanGP dependencies. Shared files are skipped automatically, so estimated sizes can be smaller when another pack is already installed.
        </p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <span className={`text-xs ${pack.installed ? "text-emerald-400" : "text-zinc-400"}`}>{pack.installed ? "Ready" : checked ? "Selected" : "Not downloaded"}</span>
              </div>
              <p className="mt-1 text-xs text-zinc-500">{pack.description}</p>
              <p className="mt-2 text-xs font-medium text-zinc-300">Approx. {pack.estimatedSize}</p>
            </button>
          );
        })}
      </div>
      ...
      {progress && (
        <div className="mt-4 rounded-lg border border-zinc-700 bg-zinc-800/60 p-3">
          <div className="flex items-center justify-between gap-3 text-xs">
            ...
            {downloading && <Loader2 className="h-4 w-4 shrink-0 animate-spin text-violet-300" />}
          </div>
          {progress.percent !== undefined ? (
            <div className="mt-2 h-1.5 overflow-hidden rounded bg-zinc-700">
              <div className="h-full bg-violet-500 transition-[width]" style={{ width: `${progress.percent}%` }} />
            </div>
          ) : downloading && (
            <div className="mt-2 h-1.5 overflow-hidden rounded bg-zinc-700">
          ...
          {progress.downloadedBytes !== undefined && (
            <p className="mt-2 text-xs text-zinc-400">{formatBytes(progress.downloadedBytes)}{progress.totalBytes ? ` / ${formatBytes(progress.totalBytes)}` : " downloaded"}{progress.speed ? ` · ${(progress.speed / 1024 / 1024).toFixed(1)} MB/s` : ""}</p>
          )}
        </div>
      )}
      ...
      <div className="mt-4 flex justify-end gap-2">
        {downloading ? (
          <Button variant="outline" className="border-zinc-600" onClick={() => void window.electronAPI.cancelModelPackDownload()}>
            <Square className="mr-2 h-3.5 w-3.5" /> Cancel download
          </Button>
        ) : (
          <>
            {firstRun && <Button variant="ghost" className="text-zinc-300" onClick={onContinue}>Skip for now</Button>}
            <Button className="bg-violet-600 hover:bg-violet-500" onClick={downloadSelected}>
              <Download className="mr-2 h-4 w-4" /> {selected.length ? `Download ${selected.length} pack${selected.length === 1 ? "" : "s"}` : firstRun ? "Continue" : "Download selected"}
            </Button>
          </>
        )}
      </div>
